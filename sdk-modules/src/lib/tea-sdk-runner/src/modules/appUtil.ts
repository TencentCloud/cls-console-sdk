...
export const appUtil = {
  setRegionId: function (rid: number) {
  ...
  getRegionId: function () {
    const { loginUin, ownerUin } = util._getLoginInfo();
    return (
      Number(localStorage.getItem(`tea_last_rid_${loginUin}_${ownerUin}`)) || 1
    );
  },
  ...
  getProjectId: function (skipAll: boolean) {
    const { loginUin, ownerUin } = util._getLoginInfo();
    let pid =
      Number(localStorage.getItem(`tea_last_pid_${loginUin}_${ownerUin}`)) ||
      -1;
    if (skipAll) {
      pid = pid == -1 ? 0 : pid;
    }
